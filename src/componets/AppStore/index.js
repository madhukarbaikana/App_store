...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, inputSearch: ''}
  ...
  render() {
    const {activeTabId, inputSearch} = this.state

    const activeCategoryApps = appsList.filter(
      eachApp => eachApp.category === activeTabId,
    )

    const filteredApps = activeCategoryApps.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(inputSearch.toLowerCase()),
    )
